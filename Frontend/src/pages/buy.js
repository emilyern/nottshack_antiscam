// =============================================================
// src/pages/buy.js — Buy DASH using MYR balance
// =============================================================
...
export default function Buy() {
  ...
  async function fetchPrice() {
    ...
    try {
      const res = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=dash&vs_currencies=myr,usd');
        ...
        <main style={{ maxWidth: '520px', margin: '0 auto', padding: '36px 24px' }}>
          <div style={card}>
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              ...
              <h2 style={{ margin: 0, color: '#f8fafc', fontSize: '22px', fontWeight: 800 }}>Purchase Successful!</h2>
              <p style={{ color: '#64748b', marginTop: '6px', fontSize: '14px' }}>DASH has been added to your wallet</p>
            </div>
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#020617' }}>
      ...
      <main style={{ maxWidth: '520px', margin: '0 auto', padding: '36px 24px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          <h1 style={{ margin: 0, fontSize: '22px', fontWeight: 800, color: '#f8fafc' }}>Buy DASH</h1>
        ...
        <div style={{ ...card, marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {myrBalance === 0 && (
            <Link to="/topup" style={{ ...btnPrimary, textDecoration: 'none', fontSize: '13px', padding: '10px 16px' }}>
              + Top Up
            </Link>
          )}
        </div>
        ...
        {/* Live price */}
        <div style={{ ...card, marginBottom: '16px', background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '4px' }}>Live DASH Price</div>
              <div style={{ fontSize: '26px', fontWeight: 800, color: '#3b82f6' }}>
                {loadingPrice ? '...' : `RM ${dashPrice?.toFixed(2)}`}
              </div>
              <div style={{ fontSize: '12px', color: '#475569' }}>≈ ${dashPriceUSD?.toFixed(2)} USD</div>
            </div>
          ...
          {lastUpdated && <div style={{ fontSize: '11px', color: '#334155', marginTop: '8px' }}>Updated: {lastUpdated.toLocaleTimeString()}</div>}
        </div>

        {/* Form */}
        <div style={card}>
          {error && <div style={errorBanner}><AlertTriangle size={15} /> {error}</div>}

          <div style={{ marginBottom: '20px' }}>
            <label style={labelStyle}>Quick Select (MYR)</label>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '8px' }}>
              {QUICK_AMOUNTS_MYR.map(amt => (
                <button key={amt} onClick={() => { setMyrAmount(String(amt)); setError(''); }}
                  style={{ padding: '6px 14px', borderRadius: '8px', fontSize: '13px', fontWeight: 600, border: myrAmount === String(amt) ? '2px solid #3b82f6' : '1px solid #1e293b', background: myrAmount === String(amt) ? '#1e3a5f' : '#020617', color: myrAmount === String(amt) ? '#60a5fa' : '#64748b', cursor: 'pointer' }}>
                  RM {amt}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{ position: 'relative', marginTop: '8px' }}>
              <input type="number" min="10" step="1" placeholder="0.00" value={myrAmount}
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ padding: '16px', borderRadius: '10px', background: '#020617', border: '1px solid #1e293b', fontSize: '22px', fontWeight: 800, color: dashAmount > 0 ? '#10b981' : '#334155', marginTop: '8px' }}>
              {dashAmount > 0 ? dashAmount.toFixed(6) : '0.000000'} DASH
            </div>
            <div style={{ fontSize: '11px', color: '#475569', marginTop: '5px' }}>
              ≈ ${dashAmount > 0 ? (dashAmount * (dashPriceUSD || 0)).toFixed(2) : '0.00'} USD • Fee: 0%
            </div>
          </div>

          <button onClick={handleBuy} disabled={loading || loadingPrice || !myrAmount || notEnoughMyr}
            style={{ ...btnPrimary, width: '100%', opacity: (loading || !myrAmount || notEnoughMyr) ? 0.5 : 1 }}>
            {loading
